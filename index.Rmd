...
final_df_neg <- bind_rows(final_df_updated, final_df) |> 
...
```{r}
language <- final_df_neg$language

buttons <- lapply(unique(final_df_neg$language), function(filter_by_this) {
  list(
    method = "update",
    args = list(
      list(
        color = list(final_df_neg$trans_status[final_df_neg$language == filter_by_this]),
        y = list(final_df_neg$package[final_df_neg$language == filter_by_this]),
        x = list(final_df_neg$percentage[final_df_neg$language == filter_by_this])
      )
    ),
    label = filter_by_this
  )
})
